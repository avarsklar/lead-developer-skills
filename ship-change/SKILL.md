...
**Glance at the other skills' dated tripwires the same way, if those files exist.** `/emergency-plan` writes `part-2-due` / `next-review-due` into `emergency-plan.md`, and `/go-live-check` writes `next check due` into `go-live-check.md` — and both tell the builder those dates "become a tripwire `/readiness-check` and `/ship-change` surface when stale." So this step-zero is where `/ship-change` keeps that promise: if one's open or past-due, mention it once, plainly, before the change — *"heads up: your emergency plan's second half (the rehearsal + down-alarm) is still to-do"* or *"your go-live check is ~4 months old; monitors rot — worth a re-run sometime."* Same rule as the 🔴 above: don't block on it, just don't let a deferred safety step rot silently unmentioned. (These files often won't exist yet — a Gate-1 builder has no emergency plan or go-live check; if they're absent, skip this silently.)
...
**Deferred to Gate 3 (named, not attempted here):** *reversible* migrations — **down-migrations** (a saved "undo" for a database-shape change, so you can reverse it without a restore), **expand-contract choreography** (making the change in safe stages: add the new column → **backfill** it, meaning fill it in for the rows that already exist → switch the app to read it → drop the old one in a later release) — so a schema change can be undone without a data restore at all. *(These jargon words are for your reasoning; the builder never needs to learn them — gloss any that surface, in one plain line each.)* This release's honest floor is "your data is recoverable from a fresh backup, and we told you the code-revert won't do it."
